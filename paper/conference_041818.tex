...
\end{subfigure}
\begin{subfigure}{0.4\textwidth}
	\includegraphics[width=\linewidth]{imgs/vir_roc_auc.png}
...
Using GPUs is an advantage for our tool and make it very fast and scalable with a large number of reads and fragments. The training process is around 5 hours with 2 million reads and the prediction is 30 seconds on 100 thousand reads using Nvidia GeForce GTX 1080 Ti. On the other hand, VirFinder processing the reads on a single CPU which takes 16X VirNet. We can avoid that by using parallel threads for VirFinder but in case you want to retrain it with new data, it will take a few days.
...
VirNet tool can classify preprocessed input genomes into a viral or non-viral sequence with an accuracy of more than 0.83 which would help us in expanding our knowledge in natural viral communities.
